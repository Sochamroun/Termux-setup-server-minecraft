# Termux-setup-server-minecraft
## Install openjdk-17 openjdk-21
```bash
curl -sL https://github.com/Sochamroun/Termux-setup-server-minecraft/raw/refs/heads/main/server-setup.sh | bash
```
## Download script install-setup
```bash
curl -L -o server-install.sh https://raw.githubusercontent.com/Sochamroun/Termux-setup-server-minecraft/ea1e43c991e4e552c4365cb90eead8c386b6d3d8/server-install.sh
```
